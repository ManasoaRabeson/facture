...
export function AddFolder({ clientId,unites,setSumFolder}) {
  ...
  const fetchProjets = async (dossierId) => {
   try {
    const response = await axios.get(`http://127.0.0.1:8000/api/cfp/factures/projets/${dossierId}`,{
          headers: {
    ...
    ...prevProjets,
    ...response.data.map((projet, index) => ({
    ...projet,
    ...
    setItemProDossier((prev) => prev + response.data.length);
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(`http://127.0.0.1:8000/api/cfp/factures/dossiers/${clientId}`,{
          headers: {
            Authorization: `Bearer ${sessionStorage.getItem('token')}`
          }
          });
